...
Mat4 view(const Vec3& pos, const Vec3& center, const Vec3& up) {
	Vec3 dir_vec = center - pos;
	float dir_len = dir_vec.norm();
	if (dir_len < 1e-6f) {
		// If camera position equals center, use default forward direction
		dir_vec = Vec3(0, 0, -1);
		dir_len = 1.0f;
	}
	Vec3 dir = dir_vec / dir_len;
	
	Vec3 u_vec = dir.cross(up);
	float u_len = u_vec.norm();
	if (u_len < 1e-6f) {
		// If dir and up are parallel, use default right direction
		u_vec = Vec3(1, 0, 0);
		u_len = 1.0f;
	}
	Vec3 u = u_vec / u_len;
